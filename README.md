...
| 原则 | 说明 |
|---|---|
| **低摩擦接入** | `services.AddEasyCoreEventBus(...)` 一条扩展方法即可 |
...
![architecture-cn](https://cdn.jsdelivr.net/gh/RockyWang0521/EasyCore.EventBus@master/docs/svg/architecture-cn.svg)

### 2.2 消息生命周期

![sequence-cn](https://cdn.jsdelivr.net/gh/RockyWang0521/EasyCore.EventBus@master/docs/svg/sequence-cn.svg)
...
```text
是否需要跨进程 / 跨服务？
├── 否 → 仅 services.AddEasyCoreEventBus() + ILocalEventBus
└── 是 → 选已有中间件
...
A: 不是。正确入口是 `services.AddEasyCoreEventBus(options => { ... })`。

**Q: 只想用本地事件，还要装 RabbitMQ 包吗？**  
A: 不需要。仅引用 `EasyCore.EventBus` 并调用 `services.AddEasyCoreEventBus()` 即可。
...
**MIT** — 详见 [LICENSE](LICENSE) 与各包 `PackageLicenseExpression`。
